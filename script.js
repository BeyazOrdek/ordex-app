...
acceptCallBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        console.error(err); 
        alert('Kamera veya mikrofona erişilemedi! Lütfen izin verdiğinizden emin olun.');
        endDmCall();
        return;
    }
...
function setupRoomUI(type) {
    ...
    const discordSidebar = document.getElementById('discord-room-sidebar');
    const discordRoomName = document.getElementById('discord-room-name');
    
    if (discordSidebar) {
        if (type === 'discord') {
            discordSidebar.classList.remove('hidden');
            if (discordRoomName) discordRoomName.textContent = currentRoom;
        } else {
            discordSidebar.classList.add('hidden');
        }
    }
...
shareScreenBtn.addEventListener('click', async () => {
    if (!isScreenSharing) {
        try {
            ...
            console.error(err); 
            alert('Ekran paylaşımı iptal edildi veya yetki verilmedi.');
        }
    } else {
        stopScreenSharing();
    }
...
function handleVideoLoading(url) {
    ...
    if (ytId) {
        ...
        const checkAndLoadYT = () => {
            if (ytPlayer && typeof ytPlayer.loadVideoById === 'function') {
                ...
                ytPlayer = new YT.Player('yt-player', {
                    ...
                    events: { 
                        'onReady': (event) => { if(isHost) event.target.playVideo(); },
                        'onStateChange': onYoutubePlayerStateChange 
                    }
                });
            } else {
                setTimeout(checkAndLoadYT, 500);
            }
        };
        checkAndLoadYT();
    } else {
        ...
        videoPlayer.load();
        if (isHost) {
            videoPlayer.play().catch(e => console.log('Auto-play prevent', e));
        }
    }
}
...
if (mobileNavToggle) {
    mobileNavToggle.addEventListener('click', () => {
        ...
        const roomDiscordSidebar = document.getElementById('discord-room-sidebar');
        
        if (serverBar) serverBar.classList.toggle('show-mobile');
        if (discordSidebar) discordSidebar.classList.toggle('show-mobile');
        if (roomDiscordSidebar && !roomDiscordSidebar.classList.contains('hidden')) {
            roomDiscordSidebar.classList.toggle('show-mobile');
        }
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Auto-login if we have a saved account
    const saved = JSON.parse(localStorage.getItem('ordex_saved_accounts') || '[]');
    if (saved.length > 0) {
        const lastAccount = saved[saved.length - 1];
        if(authUsernameInput) authUsernameInput.value = lastAccount.username;
        if(authPasswordInput && lastAccount.password) {
            authPasswordInput.value = lastAccount.password;
            switchTab('login');
            authActionBtn.click();
        } else {
            enterApp(lastAccount);
        }
    }
});
...
document.addEventListener('keydown', (e) => {
    if (!roomView.classList.contains('hidden') && currentRoomType === 'watch') {
        ...
        if (e.code === 'Space') {
            ...
            } else if (!isYoutubeMode && videoPlayer) {
            ...
            if (isYoutubeMode && ytPlayer && typeof ytPlayer.getCurrentTime === 'function') {
                const newTime = ytPlayer.getCurrentTime() + 5;
                ytPlayer.seekTo(newTime, true);
                if (isHost && typeof ytPlayer.getPlayerState === 'function' && ytPlayer.getPlayerState() !== YT.PlayerState.PLAYING) {
                    socket.emit('seek_video', { room: currentRoom, time: newTime });
                }
            } else if (!isYoutubeMode && videoPlayer) {
...
setInterval(() => {
    if (isHost && !isSyncing) {
        if (isYoutubeMode && ytPlayer && typeof ytPlayer.getCurrentTime === 'function' && typeof ytPlayer.getPlayerState === 'function') {
            socket.emit('sync_video_time', { 
...
socket.on('sync_video_time', (data) => {
    if (isHost || !currentRoom) return; // Only guests should sync to host
    ...
    if (isYoutubeMode && ytPlayer && typeof ytPlayer.getCurrentTime === 'function') {
        ...
        const state = ytPlayer.getPlayerState();
        if (data.is_playing && state !== YT.PlayerState.PLAYING) {
            ytPlayer.playVideo();
        } else if (!data.is_playing && state === YT.PlayerState.PLAYING) {
            ytPlayer.pauseVideo();
        }
    } else if (!isYoutubeMode && videoPlayer) {
        const diff = Math.abs(videoPlayer.currentTime - data.time);
        if (diff > 2) {
            isSyncing = true;
            videoPlayer.currentTime = data.time;
            setTimeout(() => { isSyncing = false; }, 600);
        }
        if (data.is_playing && videoPlayer.paused) {
            videoPlayer.play().catch(() => {});
        } else if (!data.is_playing && !videoPlayer.paused) {
            videoPlayer.pause();
        }
    }
});
